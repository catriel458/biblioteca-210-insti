...
def alta_libro(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'libro':
        try:
            libro = Libro(
                ...
                img=request.POST.get('img', ''),  # URL de imagen
                estado='Disponible',
                num_inventario=1
            )
            libro.save()
            ...
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar el libro: {str(e)}')
            return redirect('alta_material')
    
    # Si no viene del formulario dinámico, redirigir a alta_material
    return redirect('alta_material')
...
def alta_mapa(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'mapa':
        try:
            mapa = Mapas(
                tipo=request.POST.get('tipo'),
                descripcion=request.POST.get('descripcion', ''),
                num_ejemplar=int(request.POST.get('num_ejemplar', 1)),
                estado='Disponible'
            )
            mapa.save()
            
            messages.success(request, f'✅ Mapa "{mapa.tipo}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar el mapa: {str(e)}')
            return redirect('alta_material')
...
def alta_multimedia(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'multimedia':
        try:
            multimedia = Multimedia(
            ...
            messages.success(request, f'✅ Multimedia "{multimedia.materia}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar multimedia: {str(e)}')
            return redirect('alta_material')
...
def alta_notebook(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'notebook':
        try:
            notebook = Notebook(
                marca_not=request.POST.get('marca_not'),
            ...
            messages.success(request, f'✅ Notebook "{notebook.marca_not} {notebook.modelo_not}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar notebook: {str(e)}')
            return redirect('alta_material')
...
def alta_proyector(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'proyector':
        try:
            proyector = Proyector(
                marca_pro=request.POST.get('marca_pro'),
            ...
            messages.success(request, f'✅ Proyector "{proyector.marca_pro} {proyector.modelo_pro}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar proyector: {str(e)}')
            return redirect('alta_material')
...
def alta_varios(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'varios':
        try:
            varios = Varios(
                tipo=request.POST.get('tipo'),
                descripcion=request.POST.get('descripcion', ''),
            ...
            messages.success(request, f'✅ Material "{varios.tipo}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar material: {str(e)}')
            return redirect('alta_material')
...
def login_view(request):
    ...
    return render(request, 'materiales/login.html', {'form': form})
...
def registro_view(request):
    ...
    return render(request, 'materiales/registro.html', {'form': form})
...
def perfil_usuario(request):
    return render(request, 'materiales/perfil_usuario.html', {'usuario': request.user})
...
def pantalla_principal(request):
    return render(request, 'materiales/pantalla_principal.html')
...
def alta_material(request):
    """
    Vista principal para mostrar el formulario dinámico de alta de material.
    Esta vista solo renderiza el template - el procesamiento se hace en las vistas individuales.
    """
...
def alta_programa(request):
    """Solo maneja datos del formulario dinámico de alta_material"""
    if request.method == 'POST' and 'tipo_material' in request.POST and request.POST.get('tipo_material') == 'programa':
        try:
            nombre_programa = request.POST.get('nombre_programa', '')
            version = request.POST.get('version', '')
            descripcion = request.POST.get('descripcion', '')
            
            # Si tienes un modelo Programa, úsalo así:
            # programa = Programa(
            ...
            # Por ahora, solo mensaje de éxito (hasta que crees el modelo Programa)
            messages.success(request, f'✅ Programa "{nombre_programa}" versión "{version}" registrado exitosamente.')
            return redirect('alta_material')
            
        except Exception as e:
            messages.error(request, f'❌ Error al registrar programa: {str(e)}')
            return redirect('alta_material')
    
    # Si no viene del formulario dinámico, redirigir a alta_material
    return redirect('alta_material')

# Agregar al final de tu views.py

@user_passes_test(es_bibliotecaria, login_url='login')
def get_material_template(request, tipo):
    """
    Vista AJAX que devuelve el template HTML para el tipo de material seleccionado
    """
    from django.http import HttpResponse
    
    templates = {
        'libro': '''
        <div data-form-type="libro">
            <div class="row mb-3">
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="clasificacion_cdu" name="clasificacion_cdu">
                </div>
                <div class="col-md-4">
                    <label for="siglas_autor_titulo" class="text-input">Siglas Autor/Título</label>
                    <input type="text" class="form-control" id="siglas_autor_titulo" name="siglas_autor_titulo">
                </div>
            </div>
            <div class="row mb-3">
                <div class="col-12">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="etiqueta_palabra_clave" class="text-input">Palabras clave</label>
                    <input type="text" class="form-control" id="etiqueta_palabra_clave" name="etiqueta_palabra_clave">
                </div>
                <div class="col-md-6">
                    <label for="observaciones" class="text-input">Observaciones</label>
        ...
        'mapa': '''
        <div data-form-type="mapa">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="tipo" class="text-input">Tipo de Mapa *</label>
            ...
            <div id="contenedor-tipos-mapa"></div>
        </div>
        ''',
        
        'multimedia': '''
        <div data-form-type="multimedia">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="materia" class="text-input">Materia *</label>
                    <input type="text" class="form-control" id="materia" name="materia" required>
                </div>
                <div class="col-md-6">
                    <label for="contenido" class="text-input">Contenido *</label>
        ...
        'notebook': '''
        <div data-form-type="notebook">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="marca_not" class="text-input">Marca *</label>
                    <input type="text" class="form-control" id="marca_not" name="marca_not" required>
                </div>
                <div class="col-md-6">
                    <label for="modelo_not" class="text-input">Modelo *</label>
                    <input type="text" class="form-control" id="modelo_not" name="modelo_not" required>
                </div>
            </div>
            <div class="row mb-3">
                <div class="col-md-4">
                    <label for="cant_ejemplares" class="text-input">Cantidad de Ejemplares</label>
                    <input type="number" class="form-control" id="cant_ejemplares" name="cant_ejemplares" value="1" min="1">
                </div>
                <div class="col-12">
                    <label for="descripcion" class="text-input">Descripción</label>
                    <textarea class="form-control" id="descripcion" name="descripcion" rows="2"></textarea>
            ...
            <div id="contenedor-ejemplares-notebook"></div>
        </div>
        ''',
        
        'proyector': '''
        <div data-form-type="proyector">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="marca_pro" class="text-input">Marca *</label>
                    <input type="text" class="form-control" id="marca_pro" name="marca_pro" required>
                </div>
                <div class="col-md-6">
                    <label for="modelo_pro" class="text-input">Modelo *</label>
                    <input type="text" class="form-control" id="modelo_pro" name="modelo_pro" required>
                </div>
            </div>
            <div class="row mb-3">
                <div class="col-md-4">
            ...
            <div id="contenedor-ejemplares-proyector"></div>
        </div>
        ''',
        
        'varios': '''
        <div data-form-type="varios">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="tipo" class="text-input">Tipo de Material *</label>
                    <input type="text" class="form-control" id="tipo" name="tipo" required>
                </div>
                <div class="col-md-6">
                    <label for="descripcion" class="text-input">Descripción</label>
            ...
            <div id="contenedor-tipos-varios"></div>
        </div>
        ''',
        
        'programa': '''
        <div data-form-type="programa">
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="nombre_programa" class="text-input">Nombre del Programa *</label>
                    <input type="text" class="form-control" id="nombre_programa" name="nombre_programa" required>
                </div>
                <div class="col-md-6">
                    <label for="version" class="text-input">Versión</label>
    ...
    template_html = templates.get(tipo.lower(), '<div class="alert alert-warning">Tipo de material no encontrado</div>')
